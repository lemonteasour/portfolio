...
export default function ExperienceCard(props: Props) {
    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-2 text-sm">
          ...
          <ul className="flex flex-row md:flex-col md:w-1/6 xl:w-1/12 list-outside md:list-disc mx-auto md:mx-4 divide-x md:divide-x-0 text-neutral-500 dark:text-neutral-400">
            {skills.map((key) => (
  ...
  return (
    <div className="flex flex-col gap-4 my-6 p-6 md:p-8 rounded-lg bg-white dark:bg-neutral-800">
      <div className="flex flex-col md:flex-row justify-between gap-2">
        <div className="flex flex-col">
          <h3 className="text-lg font-semibold">{t("name")}</h3>
          {type !== "intern" && <p>{t("title")}</p>}
        </div>
        <div className="flex flex-col md:items-end md:text-right text-sm text-neutral-500 dark:text-neutral-400">
          <span>{t("period")}</span>
          <p>{t("location")}</p>
        </div>
      </div>
